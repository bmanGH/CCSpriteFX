...
namespace MonkVG {
	...
	,	_name( 0 )
	{
		...
		switch(format) {
			...
			case VG_sRGBA_8888:
				_GL->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
				break;
			case VG_sRGB_565:
				_GL->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, 0);
				break;
			case VG_A_8:
				_GL->glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, width, height, 0, GL_ALPHA, GL_UNSIGNED_BYTE, 0);
				break;
			default:
	...
	void OpenGLImage::setSubData( const void * data, VGint dataStride,
		...
		switch(dataFormat) {
			...
			case VG_sRGBA_8888:
				_GL->glTexSubImage2D( GL_TEXTURE_2D, 0, x, y, width, height, GL_RGBA, GL_UNSIGNED_BYTE, data );
				break;
			case VG_sRGB_565:
				_GL->glTexSubImage2D( GL_TEXTURE_2D, 0, x, y, width, height, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, data );
				break;
			case VG_A_8:
				_GL->glTexSubImage2D( GL_TEXTURE_2D, 0, x, y, width, height, GL_ALPHA, GL_UNSIGNED_BYTE, data );
				break;
			default:
				IContext::instance().setError(VG_UNSUPPORTED_IMAGE_FORMAT_ERROR);
				assert(0);
	...
	void OpenGLImage::draw() {
		...
		if ( IContext::instance().getImageMode() == VG_DRAW_IMAGE_MULTIPLY ) {
			...
			_GL->glColor4f( color[0], color[1], color[2], color[3] );
		}
...
//		glBindTexture(GL_TEXTURE_2D, _name);
		...
		_GL->glVertexPointer(3, GL_FLOAT, 0, vertices);
		_GL->glTexCoordPointer(2, GL_FLOAT, 0, coordinates);
	...
	void OpenGLImage::drawToRect( VGint x, VGint y, VGint w, VGint h, VGbitfield paintModes ) {
		...
		// note openvg coordinate system is bottom, left is 0,0
		GLfloat		vertices[] = 
		{	static_cast<GLfloat>(x),		static_cast<GLfloat>(y),		0.0,	// left, bottom
			static_cast<GLfloat>(x+w),	static_cast<GLfloat>(y),		0.0,	// right, bottom
			static_cast<GLfloat>(x),		static_cast<GLfloat>(y+h),	0.0,	// left, top
			static_cast<GLfloat>(x+w),	static_cast<GLfloat>(y+h),	0.0 };	// right, top
