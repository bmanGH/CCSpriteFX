...
namespace MonkVG {
	...
	void OpenGLPath::clear( VGbitfield caps ) {
		...
		// delete vbo buffers
		if ( _strokeVBO != -1 ) {
			_GL->glDeleteBuffers( 1, &_strokeVBO );
			_strokeVBO = -1;
		}
		
		if ( _fillVBO != -1 ) {
			_GL->glDeleteBuffers( 1, &_fillVBO );
			_fillVBO = -1;
		}
	}
	...
	bool OpenGLPath::draw( VGbitfield paintModes ) {
		...
		if( (paintModes & VG_FILL_PATH) && _fillVBO != -1 && _fillPaintForPath) {
			...
			_GL->glBindBuffer( GL_ARRAY_BUFFER, _fillVBO );
			if ( _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_COLOR ) {
				_GL->glVertexPointer( 2, GL_FLOAT, sizeof(v2_t), 0 );
				...
				_GL->glVertexPointer( 2, GL_FLOAT, sizeof(textured_vertex_t), (GLvoid*)offsetof(textured_vertex_t, v) );
				_GL->glTexCoordPointer( 2, GL_FLOAT, sizeof(textured_vertex_t), (GLvoid*)offsetof(textured_vertex_t, uv) );
			}
			_GL->glDrawArrays( GL_TRIANGLES, 0, _numberFillVertices );
		...
		if ( (paintModes & VG_STROKE_PATH) && _strokeVBO != -1 ) {
			...
			_GL->glBindBuffer( GL_ARRAY_BUFFER, _strokeVBO );
			_GL->glVertexPointer( 2, GL_FLOAT, sizeof(v2_t), 0 );
			_GL->glDrawArrays( GL_TRIANGLE_STRIP, 0, _numberStrokeVertices );
			_GL->glBindBuffer( GL_ARRAY_BUFFER, 0 );
		}
...
//					c[2] = 0;
	...
	void OpenGLPath::endOfTesselation( VGbitfield paintModes ) {
		...
		if ( _vertices.size() > 0 ) {
			...
			_GL->glGenBuffers( 1, &_fillVBO );
			_GL->glBindBuffer( GL_ARRAY_BUFFER, _fillVBO );
			if ( _fillPaintForPath && _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_COLOR ) {
				_GL->glBufferData( GL_ARRAY_BUFFER, _vertices.size() * sizeof(float), &_vertices[0], GL_STATIC_DRAW );
			} else if ( _fillPaintForPath && (_fillPaintForPath->getPaintType() == VG_PAINT_TYPE_LINEAR_GRADIENT || _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_RADIAL_GRADIENT || _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_RADIAL_2x3_GRADIENT || _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_LINEAR_2x3_GRADIENT) ) {
				...
				_GL->glBufferData( GL_ARRAY_BUFFER, texturedVertices.size() * sizeof(textured_vertex_t), &texturedVertices[0], GL_STATIC_DRAW );
		...
		if ( _strokeVertices.size() > 0 ) {
			...
			_GL->glGenBuffers( 1, &_strokeVBO );
			_GL->glBindBuffer( GL_ARRAY_BUFFER, _strokeVBO );
			_GL->glBufferData( GL_ARRAY_BUFFER, _strokeVertices.size() * sizeof(float) * 2, &_strokeVertices[0], GL_STATIC_DRAW );
