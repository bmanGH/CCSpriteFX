...
namespace MonkVG {
	...
	void OpenGLBatch::finalize() {
		...
		_GL->glGenBuffers( 1, &_vbo );
		_GL->glBindBuffer( GL_ARRAY_BUFFER, _vbo );
		_GL->glBufferData( GL_ARRAY_BUFFER, _vertices.size() * sizeof(vertex_t), &_vertices[0], GL_STATIC_DRAW );
	...
	void OpenGLBatch::draw() {
		...
		// draw
		_GL->glBindBuffer( GL_ARRAY_BUFFER, _vbo );
		_GL->glVertexPointer( 2, GL_FLOAT, sizeof(vertex_t), (GLvoid*)offsetof(vertex_t, v) );
		_GL->glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(vertex_t), (GLvoid*)offsetof(vertex_t, color) );
		_GL->glDrawArrays( GL_TRIANGLES, 0, (GLsizei)_vertexCount );
